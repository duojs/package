 ...
 *
 * This is where it writes the folder 
 * containing the downloaded package,
 * e.g. `components/duo-package@0.4.0`.
 *
 * @return {String}
 * @api public
 */
 ...
 * 1. Makes sure you have a GitHub token (that it loaded from ~/.netrc)
 * 2. If already has a fully resolved path, with a valid version and range,
 *    then consider it already resolved.
 * 3. Try finding the path from the cache.
 * 
 *
 * @return {String}
 * @api public
...
Package.prototype.resolve = unyield(function *() {
  ...
  var valid = semver.valid(this.ref) || !semver.validRange(this.ref);
  if (valid) {
    ...
    // resolved
    if (this.resolved) this.debug('resolved from cache %s', this.resolved);
...
Package.prototype.fetch = unyield(function *() {
  ...
  if (yield this.exists()) {
    ...
    if (cacheable) yield this.write(store, cache);
